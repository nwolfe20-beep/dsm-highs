...
# NWWS-OI RUNS AS A FAILOVER PAIR — DO NOT HARDCODE ONE INSTANCE.
#
# 2026-07-27: the bot dialled nwws-oi-cprk.weather.gov directly and went dead
# with TCP timeouts after working fine the day before. NOAA had failed the
# site over toward Boulder that morning. Pinning one host means every planned
# failover looks like a total outage.
#
# NWS guidance: connect to the top-level domain and let DNS resolve to
# whichever instance is live. nwws-oi.weather.gov -> bldr or cprk.
# The two explicit hosts stay in the list as fallbacks in case the top-level
# record itself is mid-update.
#
# NWS also warns of planned outages up to three times a month (10 min to 4
# hours) and unplanned ones past 10 hours, so reconnect logic has to be
# patient rather than assume a dead socket means a dead deployment.
NWWS_DOMAIN = 'nwws-oi.weather.gov'          # XMPP domain — always this
NWWS_SERVERS = [s.strip() for s in os.environ.get(
    'NWWS_OI_SERVERS',
    'nwws-oi.weather.gov,'
    'nwws-oi-cprk.weather.gov,'
    'nwws-oi-bldr.weather.gov').split(',') if s.strip()]
NWWS_PORT = 5222
...
def xmpp_connect():
    # Rotates hosts and backs off. NWWS-OI outages routinely run hours, so a
    # flat 30s retry just hammers a host that is legitimately down — and a
    # reconnect storm is a good way to get an account throttled.
    _attempt = [0]
    while True:
        sock = None
        try:
            server = NWWS_SERVERS[_attempt[0] % len(NWWS_SERVERS)]
            log.info(f"Connecting to {server}:{NWWS_PORT} "
                     f"(attempt {_attempt[0] + 1})...")
            raw = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            raw.settimeout(30)
            raw.connect((server, NWWS_PORT))
            ...
            send(raw, f'<?xml version="1.0"?>'
                      f'<stream:stream to="{NWWS_DOMAIN}" '
            ...
            log.info(f"Joined {NWWS_ROOM} as {NWWS_NICK} on {server} — listening...")
            _attempt[0] = 0        # connected: clear the backoff
            ...
            if sock:
                try:
                    ...
                    pass
        _attempt[0] += 1
        # 30s, 60s, 120s, 240s, capped at 5min. Enough to ride out a planned
        # failover without turning into a reconnect storm.
        wait = min(30 * (2 ** min(_attempt[0] - 1, 4)), 300)
        if _attempt[0] == len(NWWS_SERVERS) * 2:
            # every host tried twice — this is an outage, not a blip. Say so,
            # once, instead of silently looping for hours.
            telegram("🛑 <b>NWWS-OI unreachable</b>\n"
                     f"Tried {', '.join(NWWS_SERVERS)} twice.\n"
                     "NWS runs planned outages up to 3x/month (10min-4hr) and "
                     "unplanned ones past 10hr. Check "
                     "weather.gov/nwws for notices.")
        log.warning("Reconnecting in %ds (attempt %d)...", wait, _attempt[0])
        time.sleep(wait)
